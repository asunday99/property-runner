...
if idx == -1:
    ...
    exit(1)
# find closing div for tab-dashboard
open_divs = 0
end_idx = -1
...
dashboard_html = dashboard_html.replace('class="tab-content active space-y-6"', 'class="space-y-6"') # remove tab-content to avoid display issues
...
def extract_func(name, js_text):
    match = re.search(rf'function {name}\s*\(.*?\)\s*\{{', js_text)
    if not match: return ""
    start = match.start()
    open_braces = 0
    in_string = False
